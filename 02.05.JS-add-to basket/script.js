books = [
  ...
  let cardArea=document.querySelector('.card')
  function createCard(smth) {
    smth.forEach((element) => {
      let divElem = document.createElement("div");
      divElem.classList.add("border")
      divElem.innerHTML = `
      <h5 class="card-title">Title: ${element.title}</h5>
      <p class="card-authors">authors:${element.authors}</p>
      <p class="card-publisher">publisher:${element.publisher}</p>
      <p class="card-year">year:${element.year}</p>
      <button class="btn btn-primary">Add to card</button>
          `;
      cardArea.append(divElem);
    });
  }
